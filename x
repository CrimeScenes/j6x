...
local function predictTargetPosition(targetPlayer, deltaTime)
    ...
    if humanoidRootPart then
        ...
        local velocity = (lastPosition - (targetPlayer.Character:FindFirstChild("HumanoidRootPart") or lastPosition)).Magnitude / deltaTime
        -- Predict the future position based on velocity
        local predictedPosition = lastPosition + (velocity * deltaTime)
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinViewEra(part.Position) then
                        -- Apply prediction
                        local deltaTime = RunService.Heartbeat:Wait() -- Time passed since last frame
                        local predictedPosition = predictTargetPosition(TargetPlayer, deltaTime)
